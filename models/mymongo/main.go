...
func GetMongoInfoByDigest(digest string) string {
	...
	query := func(c *mgo.Collection) error{//匿名函数
		return c.Find(bson.M{"company_name_digest":digest,"deleted":bson.M{"ne":1}}).One(&aObj)
	}
...
func GetMongoInfoByCompanyName(company_name string) string {
	var aObj digest
	query := func(c *mgo.Collection) error{//匿名函数
		return c.Find(bson.M{"company_name":company_name,"deleted":bson.M{"ne":1}}).One(&aObj)
	}
	err := handlerCollection("ic",query)
	if err != nil{
		fmt.Println(err)
